  ...
  return (
    <div className="border border-gray-300 rounded-xl p-6 bg-white shadow-md hover:shadow-lg transition-shadow h-full flex flex-col">
      ...
      <div className="text-center space-y-1 flex-1 flex flex-col justify-center">
        ...
        {/* Availability badge */}
        <div className="mt-2">
          <span className={`inline-flex items-center px-2 py-1 rounded text-xs ${card.avail ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}>
            {card.avail ? 'Available' : 'Unavailable'}
          </span>
        </div>
      </div>
      ...
      <div className="mt-4 flex justify-center">
        ...
        <button
          onClick={async (e) => {
            e.stopPropagation();
            try {
              const updated = await updateCard(card.id, { avail: !card.avail });
              if (updated && updated.card) {
                onCardUpdate && onCardUpdate(card.id, { ...card, avail: updated.card.avail });
              }
            } catch (err) {
              console.error('Failed to toggle avail:', err);
              alert('Failed to update availability');
            }
          }}
          className={`ml-3 px-4 py-2 rounded-lg text-base font-medium transition-colors cursor-pointer ${card.avail ? 'bg-red-500 hover:bg-red-600 text-white' : 'bg-green-500 hover:bg-green-600 text-white'}`}
          title="Toggle availability"
        >
          {card.avail ? 'Make Unavailable' : 'Make Available'}
        </button>
      </div>
    </div>
  );
